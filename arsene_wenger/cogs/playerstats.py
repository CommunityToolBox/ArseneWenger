...
from arsene_wenger.utils import current_season
...
class PlayerStatsCog(commands.Cog):
    ...
    async def goals(self, interaction: discord.Interaction, team: str = "Arsenal"):
        ...
        try:
            goals = get_goals_scored(team_id)
    ...
    async def assists(self, interaction: discord.Interaction, competition: str = "pl"):
        ...
        assists = get_assists(competition_name)
    ...
    @app_commands.command(name="injuries", description="Get the list of injuries and details about them")
...
def get_player_stats(club_id):
    ...
    for each in rows_player:
        if each.find("th", {"scope": "row"}) is None:
...
def get_goals_scored(club):
    df_topscorers = get_player_stats(club)
